...
/**
 * Calculates rise & set times of the sun or moon, and also calculates twilight
 * times (civil/nautical/astronomical).
 * <P>
 ...
 * during a given day starting on the given Julian day. It does this by
 * computing the altitude of the object during each of the 24 hours of that day
 * (and 1 hour of the next). What we really want to know is the object's
 * altitude relative to the "rise/set altitude." This is the altitude at which
 * the top of the object becomes visible, after correcting for refraction and
 * (in the case of the Moon) topocentric parallax.
 * <P>
 * For the sun, this altitude is -.8333 degrees (its apparent radius is about
 * .25 degrees, and refraction 'lifts it up' by .58333 degrees.) For the moon,
 * this altitude is +.125 degrees.
 * <P>
 * If we find that the object was below this altitude at one hour, and above it
 * on the next hour, then it must have risen in that interval; Conversely, if we
 * find that the object was above this altitude at one hour, and below it on the
 * next hour, then it must have set in that interval.
 * <P>
 * We then do an iterative search to find the instant during that hour that it
 * rose or set. This starts with a guessed rise/set time in the middle of the
 * particular hour in question. At each step, we look at the altitude of that
 * object at that time, and use it to adjust the rise/set time based on the
 * assumption that the motion was linear during the hour (this isn't a perfect
 * assumption, but we still usually converge in a few iterations.)
 * <P>
 * As a side benefit, this function will also calculate twilight times by using
 * the sun and just changing the event altitude. The modified altitudes are -6
 * degrees (civil twilight), -12 degrees (nautical twilight) and -18 degrees
...
public class RiseSet
{
     ...
     *
     * @param rsType Rise/Set type (SUN, MOON, CIVIL_TWI, NAUTICAL_TWI, or
     * ASTRONOMICAL_TWI).
     * @param jd The Julian day for which to calculate the desired information
     * (set to 0h local time
